...
def cantidad_superheroes_por_genero(genero:str)->int:
  """Determina cuántos superhéroes tienen cada tipo de color de ojos."""
  cantidad = 0
  cantidad_por_color = {}

  for superheroe in lista_personajes:
    # Obtenemos el color de ojos del superhéroe.
    color_genero = superheroe[genero]

    # Si el color de ojos no está en el diccionario, lo añadimos.
    if color_genero not in cantidad_por_color:
      cantidad_por_color[color_genero] = 0

    # Incrementamos la cantidad de superhéroes con este color de ojos.
    cantidad_por_color[color_genero] += 1

  # Recorremos el diccionario para imprimir la cantidad de superhéroes por color de ojos.
  for color_genero, cantidad in cantidad_por_color.items():
    if cantidad > 1:
      print(f"Hay {cantidad} superhéroes con género {genero}: {color_genero}")
    else:
      print(f"Hay {cantidad} superhéroe con género {genero}: {color_genero}")
  return cantidad


def superheroes_agrupados_por_genero(genero:str)->dict:
  """Lista todos los superhéroes agrupados por tipo de inteligencia."""
...
def mostrar_superheroes_agrupados_tipo_inteligencia(superheroes_por_tipo_inteligencia:dict):
  print("+--------------------+-------------------------------+---------------+--------+--------+--------+-------------------------+---------------------+--------+--------------+")
  print(f"| Nombre\t     | Identidad\t\t     | Empresa\t     | Altura | Peso   | Género | Ojos\t\t\t  | Pelo\t\t| Fuerza | Inteligencia |")
  print("+--------------------+-------------------------------+---------------+--------+--------+--------+-------------------------+---------------------+--------+--------------+")
  for tipo_inteligencia in superheroes_por_tipo_inteligencia:
    for superheroe in superheroes_por_tipo_inteligencia[tipo_inteligencia]:
...
def mostrar_superheroes_agrupados_color_ojos(superheroes_por_color_ojos:dict):
  print("+--------------------+-------------------------------+---------------+--------+--------+--------+-------------------------+---------------------+--------+--------------+")
  print(f"| Nombre\t     | Identidad\t\t     | Empresa\t     | Altura | Peso   | Género | Ojos\t\t\t  | Pelo\t\t| Fuerza | Inteligencia |")
  print("+--------------------+-------------------------------+---------------+--------+--------+--------+-------------------------+---------------------+--------+--------------+")  
  for tipo_color_ojos in superheroes_por_color_ojos:
    for superheroe in superheroes_por_color_ojos[tipo_color_ojos]:
      print(f"| {superheroe['nombre']:18} | {superheroe['identidad']:29} | {superheroe['empresa']:10} | \
{float(superheroe['altura']):6.2f} | {float(superheroe['peso']):6.2f} | {superheroe['genero']:6} | \
{superheroe['color_ojos']:23} | {superheroe['color_pelo']:19} | {superheroe['fuerza']:6} | \
{superheroe['inteligencia']:12} |")
...
#   """Procesa la opción elegida por el usuario."""
    match opcion:
      ...
      case "6":
        fuerza_promedio_nb()
      case "7":
        cantidad_superheroes_por_genero("color_ojos")
      case "8":
        cantidad_superheroes_por_genero("color_pelo")
      case "9":
        superheroes_agrupados_por_color_ojos = superheroes_agrupados_por_genero("color_ojos")
        mostrar_superheroes_agrupados_color_ojos(superheroes_agrupados_por_color_ojos)
      case "10":
        superheroes_agrupados_por_tipo_inteligencia = superheroes_agrupados_por_genero("inteligencia")
        mostrar_superheroes_agrupados_tipo_inteligencia(superheroes_agrupados_por_tipo_inteligencia)
      case "11":
